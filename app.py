...
Everything for the web layer (HTML, CSS, JS) lives in this one file so the
repo only needs app.py + the protected/ folder. The ML pipeline itself is
NOT reimplemented anywhere below: it is loaded from protected/fraud_detection_model.pkl
exactly as you saved it with joblib, and .predict() / .predict_proba() are
called directly on that object.

Your original files are treated as read-only and live in protected/:
  - fraud_detection_model.pkl  (loaded with joblib.load, never modified)
  - credit_card_fraud_10k.csv  (only read, to power the "Load example" button)
